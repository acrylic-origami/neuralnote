...
export default class DocumentEditor extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState, history) {
    if (prevProps.location.pathname !== this.props.location.pathname)
      this.setState({ editorState: this.decideContent() });
  }

  componentDidMount() {
    if (this.state.searched === false) {
      this.setState({ editorState: this.decideContent(), searched: true });
    }
  }
  ...
  decideContent = () => {
    ...
    let foundInState = this.state.documents.find(function(document) {
      return document.name === documentTitle;
    });
    if (foundInState) return this.renderForEditor(foundInState.content);
